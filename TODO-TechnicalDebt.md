...
- [x] Stop older builds deleting fields they do not recognise. `sanitizeDocument` rebuilt `data` from a fixed list of known keys and dropped everything else, and the merge did the same, so any build that predated a new field would strip it and sync the stripped document back — erasing it for every one of the user's devices. Both now carry unknown fields through untouched, minus a `RETIRED_DATA_FIELDS` list for fields deliberately removed from the document (`embuDraft`). Alongside it, clients declare the schema they understand in `X-App-Schema-Version` and the server refuses writes older than the stored document with 409, which is the only place this can be enforced — the dangerous builds are the ones already installed. The gate is dormant until a release actually writes a newer shape
