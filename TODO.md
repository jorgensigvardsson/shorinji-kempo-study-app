...
- [ ] Fix the `hokei_name` collision in notes and star ratings. Both are keyed by `hokei.hokei_name` alone (`useHokeiNote`/`useHokeiRank` in `persistence/app-data.ts`, called from `HokeiCard.tsx`), and that name is not unique in `kamokuhyo.json`: 225 distinct names across 237 entries. Eleven names are shared by two or three genuinely different hokei moments, told apart only by `variations` — and sometimes by grade, so a collision can span two grades:
  ...
  The effect is live today: a note written on one variation shows up on the others, and rating one rates all of them. It is silent in both directions — nothing warns the user, and nothing in the data marks the entries as distinct.

  The fix is a stable identity for a hokei moment rather than a wider key: `variations` is enough to separate today's collisions, but a key built from display text keeps this class of bug alive, since correcting a spelling in the source then orphans whatever was saved against the old text. Whatever is chosen has to migrate the keys already stored in users' synced documents, since nothing garbage-collects a key whose content no longer exists — see the identity discussion in `TODO-TechnicalDebt.md` before designing it, and note that per-note storage would turn these keys into Cosmos item ids, which are the hardest thing to change afterwards
