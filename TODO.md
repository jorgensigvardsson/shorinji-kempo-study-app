...
- [ ] Make every technique in grading Kumi-embu open the same full technique card as it does in Free practice. The shared sequence renderer already supports clickable names, but grading currently builds its lookup only from the selected grade and keys it by `hokei_name`; sequences often refer to techniques from earlier grades and to specific variations, so visible names can remain unresolved. Reuse one variation-aware, all-relevant-grades technique catalogue in both contexts, while preserving notes, ratings, ids and Dojo presentation.
- [ ] Investigate the missing fixed Kumi-embu sequences for yondan, godan and rokudan against the authoritative grading material before adding anything. `grading-exam-information.json` currently contains fixed Kumi-embu only for 3 kyū through sandan, and Free practice deliberately derives its available sequences from those entries; the higher-grade technical sections contain no Kumi-embu item at all. Establish whether that omission is intentional at those levels or whether source content is missing—never invent a sequence to fill the gap.
- [ ] Streamline Malin's frontend iteration loop: keep the already-started local services running, use Vite hot reload and focused tests while experimenting, and reserve the full test suite and production build for a meaningful checkpoint. GitHub collaborator status affects fetch/push permissions, not local compile speed. Frontend-only work should not modify backend code unless the requested feature genuinely requires it and the scope has been explained and approved first.
...
- [x] Rework the weekly plan's focus headings: remove the "Veckans innehåll" section label entirely, and restyle the group headings ("Veckans grundarbete", "Återkommande delar", "Repetitionstekniker", …) with the calm small-caps eyebrow look that label uses today, instead of the large serif display style. Keep the section's accessible name when the visible `h2` disappears
- [x] Merge `test.yml` and `deploy-staging.yml`/`deploy.yml` into a single workflow triggered directly by `push`, instead of linking them via `workflow_run`. Done: `ci.yml` is now the only workflow with a trigger. It runs the test jobs on every push and gates both deploys behind them with `needs:`, calling `deploy-staging.yml`/`deploy.yml` — now reusable `workflow_call` workflows — with `uses: ./…`, which resolves against the pushed branch's own commit. That fixes both problems: a red test now stops a production deploy (`deploy.yml` used to trigger on `push` in parallel with Test, leaving the weaker gate on the more important environment), and a deploy-workflow change now takes effect on the branch that made it instead of only after a merge to `main` (which silently dropped a new `FEEDBACK_EMAIL` parameter from a staging deploy on 2026-08-13). `ci.yml`'s `workflow_dispatch` also deploys any chosen branch to staging or production on demand. Verified by a real staging deploy on 2026-08-16, which also flushed out the Azure side: the OIDC subject is now the pushed branch, so a third federated credential (`ref:refs/heads/deploy-staging`) had to be added — the existing one named `github-deploy-staging` carries the `main` subject and belongs to `renew-certs.yml`, not to staging. See BACKEND.md's "OIDC federated credentials" before touching any of them.
- [x] Fix the `hokei_name` collision in notes and star ratings. Both are keyed by `hokei.hokei_name` alone (`useHokeiNote`/`useHokeiRank` in `persistence/app-data.ts`, called from `HokeiCard.tsx`), and that name is not unique in `kamokuhyo.json`: 225 distinct names across 237 entries. Eleven names are shared by two or three genuinely different hokei moments, told apart only by `variations` — and sometimes by grade, so a collision can span two grades:
  - `tsuki nuki` (soto / uchi / ryōte — three entries, different foot stances and different videos, one of them in another grade)
  - `ryūsui geri`, `uchi uke zuki`, `uwa uke zuki`, `uwa uke geri`, `soto uke zuki`, `soto uke geri`, `uchi uke geri` (all niō ken)
  - `maki nuki`, `kiri nuki`, `jūji nuki` (all ryūō ken)

  The effect was live rather than theoretical: two production notes were found sitting on shared keys.

  Fixed by giving each moment an `id` — the `hokei_name` where it belongs to one moment, and `name [variation]` for the 23 that shared one. The square brackets are the whole trick, and they came from the data rather than from taste: an earlier version of the app wrote note keys in exactly that form and those keys are still stored, so `uchi uke zuki [ura]` lands on its new id untouched. The first draft used parentheses and would have orphaned precisely the notes this was written to protect — which is the lesson worth keeping. Look at what is actually stored before choosing a key format.

  No migration and no compatibility layer. An earlier draft split old shared keys across the moments that shared them, and lost the shared note outright when every variation already had one of its own; it bought nothing the ids do not. The two genuinely ambiguous keys that remain belong to a test user, and a new collision is caught by the id uniqueness test in `kamokuhyo.test.ts` rather than by a list somebody has to maintain
