...
- **Flashcards** — choose between terminology cards and grade-based Hokei cards whose backs show stance, execution, and synced personal notes (non-Japanese interface languages)
...
mid-word alike. "Default" now means Open Sans for body text, Lora for headings, and
Shippori Mincho for kanji/kana. The kanji picker starts filtered to the Japanese language
subset, since a font without kanji glyphs would appear to do nothing at all.
`frontend/src/index.css` documents how the layers fit together, and `AGENTS.md` covers what
this means when writing new CSS.

Loading a font reaches two Google origins — `fonts.googleapis.com` for the stylesheet and
`fonts.gstatic.com` for the font files it references. The default fonts use those origins
in every environment, so [`frontend/public/.htaccess`](frontend/public/.htaccess) allows
them in the deployed `Content-Security-Policy`. Local `npm run dev` sends no CSP.
