...
func main() {
	...
	acsEndpoint  := flag.String("acs-endpoint",       envutil.String("ACS_ENDPOINT",       ""), "Azure Communication Services endpoint (enables email sending when set with key+sender)")
	acsAccessKey := flag.String("acs-access-key",     envutil.String("ACS_ACCESS_KEY",     ""), "Azure Communication Services access key")
	...
	// ── Email sender (ACS when fully configured; dev stdout otherwise) ─────────
	var mailer email.Sender
	if *acsEndpoint != "" && *acsAccessKey != "" && *acsSender != "" {
		s, err := email.NewACSSender(*acsEndpoint, *acsAccessKey, *acsSender)
		if err != nil {
			log.Fatalf("init ACS email sender: %v", err)
		}
		mailer = s
		log.Printf("email sending via ACS (sender: %s)", *acsSender)
	} else {
		mailer = email.LogSender{}
		log.Println("email sending disabled — verification codes will be logged to stdout (set ACS_ENDPOINT, ACS_ACCESS_KEY, ACS_SENDER_ADDRESS to enable)")
	}
