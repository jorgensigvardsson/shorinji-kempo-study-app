...
func newDocumentHandler(t *testing.T) *Handler {
	t.Helper()
	return NewHandler(store.NewFileUserDataStore(t.TempDir()), nil, "http://frontend", testIssuer, nil)
}
...
// failingStore stands in for the store being broken or unreachable.
type failingStore struct{}

func (failingStore) Load(string) (*store.Document, string, error) { return nil, "", nil }
func (failingStore) Save(string, *store.Document, string) (string, error) {
	return "", errors.New("store unavailable")
}
func (failingStore) SaveUnconditional(string, *store.Document) (string, error) {
	return "", errors.New("store unavailable")
}
func (failingStore) Delete(string) error { return errors.New("store unavailable") }

func deleteAccount(h *Handler, userID string) *httptest.ResponseRecorder {
	req := asUser(httptest.NewRequest(http.MethodDelete, "/api/v1/account", nil), userID)
	rec := httptest.NewRecorder()
	h.deleteAccount(rec, req)
	return rec
}

func TestDeleteAccount_ClearsTheDocument(t *testing.T) {
	h := newDocumentHandler(t)
	putDocument(h, "user-1", `{"version":1,"data":{"a":1}}`, clientBuild("1", "2", map[string]string{"If-None-Match": "*"}))

	if rec := deleteAccount(h, "user-1"); rec.Code != http.StatusNoContent {
...
// Failing to delete leaves data behind after someone asked for it to be gone. That
// has to be reported rather than swallowed.
func TestDeleteAccount_FailureIsReported(t *testing.T) {
	h := NewHandler(failingStore{}, nil, "http://frontend", testIssuer, nil)

	if rec := deleteAccount(h, "user-1"); rec.Code != http.StatusInternalServerError {
